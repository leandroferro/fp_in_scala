...
def joinAdding(l1: List[Int], l2: List[Int]): List[Int] = 
  (l1, l2) match {
    case (Nil, Nil) => Nil
    case (Nil, Cons(b, t2)) => Cons(b, t2)
    case (Cons(a, t1), Nil) => Cons(a, t1)
    case (Cons(a, t1), Cons(b, t2)) => Cons(a+b, joinAdding(t1, t2))
  }
